...
T = TypeVar(
    "T", None, mesh_pb2.Routing, telemetry_pb2.Telemetry, admin_pb2.AdminMessage, str
)
...
class Packet(Generic[T]):
    ...
    def from_id(self) -> int | None:
        return (
            getattr(self.mesh_packet, "from") if self.mesh_packet is not None else None
        )
    ...
    def data(self) -> mesh_pb2.Data | None:
        return (
            self.mesh_packet.decoded
            if self.mesh_packet and self.mesh_packet.HasField("decoded")
            else None
        )
    ...
    def __init__(self, database: Mapping[str, Any]) -> None:
        ...
        try:
            google.protobuf.json_format.ParseDict(
                database, from_radio.node_info, ignore_unknown_fields=True
            )
        except:  # noqa: E722
            from_radio.node_info.num = database["num"]
            google.protobuf.json_format.ParseDict(
                database["user"], from_radio.node_info.user
            )
