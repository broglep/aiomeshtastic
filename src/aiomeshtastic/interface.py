...
class MeshNode:
    ...
    def stub_node(node_id: int) -> "MeshNode":
        ...
        return MeshNode(
            id=node_id,
            user_id=user_id,
            short_name=f"{user_id[-4:]}",
            long_name=f"Meshtastic {user_id[-4:]}",
        )
...
def process_while_running(f):  # noqa: ANN001, ANN201
    ...
    async def wrapper(self: "MeshInterface") -> None:
        while self.is_running:
            try:
                self._logger.debug(
                    "Processing while running starting, function: %s", f.__name__
                )
                await f(self)
            except asyncio.CancelledError:
                self._logger.debug(
                    "Processing while running, aborting, function: %s", f.__name__
                )
                raise
            except:  # noqa: E722
                self._logger.debug(
                    "Processing while running failed, restarting soon, function: %s",
                    f.__name__,
                    exc_info=True,
                )
                try:
                    ...
                    self._logger.debug(
                        "Processing cancelled, aborting, function: %s", f.__name__
                    )
                    break
                self._logger.debug(
                    "Resuming processing while running, function: %s", f.__name__
                )
...
class MeshInterface:
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        self._heartbeat_interval_s = (
            600 if heartbeat_interval is None else heartbeat_interval.total_seconds()
        )

        self._ack_timeout = (
            30.0
            if acknowledgement_timeout is None
            else acknowledgement_timeout.total_seconds()
        )
        self._response_timeout = (
            60.0 if response_timeout is None else response_timeout.total_seconds()
        )
        ...
        self._packet_stream_listeners: list[
            ClientApiConnectionPacketStreamListener
        ] = []
        self._app_listeners: dict[
            portnums_pb2.PortNum, list[Callable[[MeshNode, Packet], Awaitable[None]]]
        ] = defaultdict(list)
    ...
    def add_packet_app_listener(
        ...
        *,
        ...
        async def wrapper(node: MeshNode, source: Packet) -> None:
            if as_dict:
                await callback(
                    node, google.protobuf.json_format.MessageToDict(source.app_payload)
                )
            elif as_packet:
        ...
        if node_id is not None:
            ...
            def matches(node_info: Mapping[str, Any]) -> bool:
                ...
                if (
                    ...
                    and node_info["user"]["shortName"] == short_name
                ):
                    return True

                return bool(
                    long_name is not None and node_info["user"]["longName"] == long_name
                )

            node_info = next(
                (
                    node_info
                    for node_info in self._node_database.values()
                    if matches(node_info)
                ),
                None,
            )
        ...
        if index is not None:
            ...
            channel = next(
                (
                    ...
                    if channel_info.settings.name == name
                    and channel_info.role != channel_pb2.Channel.Role.DISABLED
                ),
                None,
            )
            if channel is None:
    ...
    async def start(self) -> None:
        ...
        self._processing_tasks.add(
            asyncio.create_task(
                self._process_from_radio_packets_loop(),
                name="process_from_radio_packets",
            )
        )
    ...
    def _process_connected_node_module_config(
        ...
        if module_config.HasField("external_notification"):
            self._connected_node_module_config.external_notification.CopyFrom(
                module_config.external_notification
            )
        if module_config.HasField("store_forward"):
            self._connected_node_module_config.store_forward.CopyFrom(
                module_config.store_forward
            )
        if module_config.HasField("range_test"):
            self._connected_node_module_config.range_test.CopyFrom(
                module_config.range_test
            )
        if module_config.HasField("telemetry"):
            self._connected_node_module_config.telemetry.CopyFrom(
                module_config.telemetry
            )
        if module_config.HasField("canned_message"):
            self._connected_node_module_config.canned_message.CopyFrom(
        ...
        if module_config.HasField("remote_hardware"):
            self._connected_node_module_config.remote_hardware.CopyFrom(
                module_config.remote_hardware
            )
        if module_config.HasField("neighbor_info"):
            self._connected_node_module_config.neighbor_info.CopyFrom(
                module_config.neighbor_info
            )
        if module_config.HasField("detection_sensor"):
            self._connected_node_module_config.detection_sensor.CopyFrom(
                module_config.detection_sensor
            )
        if module_config.HasField("ambient_lighting"):
            self._connected_node_module_config.ambient_lighting.CopyFrom(
                module_config.ambient_lighting
            )
        if module_config.HasField("paxcounter"):
            self._connected_node_module_config.paxcounter.CopyFrom(
    ...
    async def _process_packet_for_app_listener(
        ...
        if packet.data is None:
            self._logger.debug(
                "Packet could not be decoded: %s",
                repr(packet.mesh_packet).replace("\n", ""),
            )
            return
        ...
        if packet.app_payload is None:
            self._logger.debug(
                "Packet has no payload: %s", repr(packet.data).replace("\n", "")
            )
            return
        ...
        for listener in self._app_listeners[packet.port_num]:
            self._add_background_task(
                listener(node, packet), name=f"app-listener-{packet.port_num}"
            )
    ...
    async def _process_node_info(self, packet: mesh_pb2.FromRadio) -> None:
        ...
        if packet.HasField("node_info"):
            ...
            try:
                ...
                for listener in self._app_listeners[portnums_pb2.PortNum.NODEINFO_APP]:
                    self._add_background_task(
                        listener(node, node_info_packet),
                        name=f"app-listener-{portnums_pb2.PortNum.NODEINFO_APP}",
                    )
        ...
        if p.from_id:
            await self._node_database_update(
                p.from_id, lastHeard=p.rx_time, snr=p.rx_snr
            )
    ...
    def _create_db_node(
        self, node_num: int, node_info: Mapping[str, Any] | None = None
    ...
    async def _reconnect_while_running(self, *, force: bool = False) -> None:  # noqa: PLR0915
        ...
        recent_reconnects = [
            t
            for t in self._previous_reconnects
            if (now - t) <= datetime.timedelta(seconds=30)
        ]
        if len(recent_reconnects) >= reconnect_counter_max:
            backoff = float(random.randint(30, 60))  # noqa: S311
            self._logger.debug(
                "Too many reconnects in last 30 seconds, waiting %.0f seconds", backoff
            )
        ...
        while self.is_running:
            ...
            reconnect_delay = float(
                random.randint(2**reconnect_counter, 2 ** (reconnect_counter + 1))
            )  # noqa: S311
            ...
            try:
                ...
                async with self._reconnect_lock:
                    ...
                    try:
                        await asyncio.wait_for(
                            self._connection.reconnect(force=force_reconnect),
                            timeout=30,
                        )
                    except TimeoutError:
                        self._logger.debug(
                            "Reconnect connection did timeout, retrying in %.0f seconds",
                            reconnect_delay,
                        )
                        ...
                        self._logger.debug(
                            "Reconnect connection succeeded, requesting config"
                        )

                    try:
                        await asyncio.wait_for(
                            self._connection.request_config(minimal=self.no_nodes),
                            timeout=60,
                        )
                        if not self._connected_node_ready.is_set():
                            self._logger.debug(
                                "Completed first request config as part of reconnect"
                            )
                ...
                self._logger.debug(
                    "Reconnecting failed, retrying in %.0f seconds", reconnect_delay
                )
                await asyncio.sleep(reconnect_delay)
    ...
    def _add_background_task(
        self, coro: Awaitable[None], name: str | None = None
    ) -> asyncio.Task:
    ...
    async def send_time(self, node: int | None = None) -> None:
        ...
        await self.send_admin_message_await_response(
            node=node, message=admin_message, expect_response=False
        )

    async def write_timezone_if_needed(self, node: int | None = None) -> bool:
        tz_string = await asyncio.get_running_loop().run_in_executor(
            None, self._get_tz_string
        )
    ...
    async def request_connection_status(
    ...
    async def request_lora_config(
        self, node: int | None = None
    ) -> config_pb2.Config.LoRaConfig:
        admin_message = admin_pb2.AdminMessage()
    ...
    async def write_lora_config(
        self, lora: config_pb2.Config.LoRaConfig, node: int | None = None
    ) -> None:
        admin_message = admin_pb2.AdminMessage()
        config = config_pb2.Config()
    ...
    async def _send_message_await_response(  # noqa: PLR0913
        ...
        *,
        ...
        actual_timeout = (
            ...
            else (self._response_timeout if want_response else self._ack_timeout)
        )
        ...
        async def on_ack(packet: Packet[mesh_pb2.Routing]) -> None:
            ...
            if packet.app_payload.error_reason != mesh_pb2.Routing.Error.NONE:
                error_queue.put_nowait(
                    MeshRoutingError(packet.app_payload.error_reason)
                )
        ...
        send_packet_task = asyncio.create_task(
            ...
            [abort_on_error_task, send_packet_task],
        ...
        *,
        ...
        return await self._connection.send_mesh_packet(
            ...
            or (
                ...
                if want_ack
                else MeshPacket.Priority.DEFAULT
            ),
            want_response=False,
            ack=want_ack,
    ...
    async def request_traceroute(
        self, node: int | MeshNode, timeout: float = UNDEFINED
    ) -> mesh_pb2.RouteDiscovery:  # noqa: ASYNC109
