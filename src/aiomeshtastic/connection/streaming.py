...
class StreamingClientTransport(ClientApiConnection):
                ...
                while self._can_read():
                    ...
                    try:
                        ...
                        self._logger.warning(
                            "Error while parsing FromRadio bytes %s",
                            packet,
                            exc_info=True,
                        )
    ...
    async def _read_bytes(
        self, n: int = -1, *, exactly: int | None = None
    ) -> bytes | None:
        pass
    ...
    async def _read_header(self) -> bytes | None:
        ...
        while (
            missing_magic_bytes := StreamingClientTransport.MAGIC_LEN - len(header)
        ) > 0:
        ...
        while not (
            header[0] == StreamingClientTransport.START1
            and header[1] == StreamingClientTransport.START2
        ):
            ...
            if header[1] == StreamingClientTransport.START1:
                ...
                while len(header) < StreamingClientTransport.MAGIC_LEN:
                    b = await self._read_bytes(
                        StreamingClientTransport.MAGIC_LEN - len(header)
                    )
                    if b is not None:
                        header += b
            else:
        ...
        while (
            missing_header_bytes := StreamingClientTransport.HEADER_LEN - len(header)
        ) > 0:
