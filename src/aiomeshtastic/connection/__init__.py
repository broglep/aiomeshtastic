...
class ClientApiConnection:
    ...
    async def listen(
        self, on_start: Coroutine | None = None
    ) -> AsyncIterable[mesh_pb2.FromRadio]:
        if not self.is_connected:
    ...
    async def nodes(self) -> AsyncIterable[mesh_pb2.NodeInfo]:
        async for packet in self.listen():
            if packet.HasField("node_info"):
                self._logger.debug(
                    "Received node info: %s", self._protobuf_log(packet.node_info)
                )
    ...
    async def _update_queue_status(self, packet: mesh_pb2.FromRadio) -> None:
        if packet.HasField("queueStatus"):
            ...
            self._logger.debug(
                "New Queue Status: %s", repr(self._queue_status).replace("\n", "")
            )

    async def _notify_packet_stream_listeners(
        self, packet: mesh_pb2.FromRadio, *, sequential: bool = False
    ) -> None:
        async def notify(
            listener: ClientApiConnectionPacketStreamListener,
            new_packet: mesh_pb2.FromRadio,
        ) -> None:
            try:
                await listener.notify(new_packet)
            except:  # noqa: E722
                self._logger.warning(
                    "Listener notify failed: %s", listener, exc_info=True
                )
        ...
        if sequential:
            ...
            await asyncio.wait(
                [
                    asyncio.create_task(notify(listener, packet))
                    for listener in self._packet_stream_listeners
                ]
            )
    ...
    def extract_mesh_packet_data(
        ...
        if not packet.HasField("decoded"):
            self._logger.debug(
                "Packet could not be decoded: %s", self._protobuf_log(packet)
            )
            return None
    ...
    async def send_mesh_packet(  # noqa: PLR0913
        ...
        *,
        ...
        ack_callback: Callable[[Packet[mesh_pb2.Routing]], Awaitable[None]]
        | None = None,
    ...
    async def send_packet(self, to_radio: mesh_pb2.ToRadio) -> bool:
        ...
        while self._queue_status.free == 0:
            ...
            queue_status_update_task = asyncio.create_task(
                self._queue_status_update.wait()
            )
            sleep_task = asyncio.create_task(asyncio.sleep(1.0))
            await asyncio.wait(
                [queue_status_update_task, sleep_task],
                ...
                try:
                    while not self._on_demand_streaming_processing_stop.is_set():
                        try:
                            ...
                            self._logger.debug(
                                "On demand processing failed with connection error",
                                exc_info=True,
                            )
                            if self._reconnect_in_progress.is_set():
                                self._logger.debug(
                                    "On demand processing, waiting for reconnect"
                                )
                                await self._reconnect_completed.wait()
                                self._logger.debug(
                                    "On demand processing, reconnect done"
                                )
                            else:
                                with contextlib.suppress(asyncio.CancelledError):
                            ...
                            self._logger.warning(
                                "On demand processing failed with unexpected error",
                                exc_info=True,
                            )
    ...
    async def reconnect(self, *, force: bool = False) -> bool:
        ...
        if reconnect_was_in_progress:
            ...
            failed_task = asyncio.create_task(self._reconnect_failed.wait())
                ...
                [complete_task, failed_task], return_when=asyncio.FIRST_COMPLETED
            )
            if done == {failed_task}:
