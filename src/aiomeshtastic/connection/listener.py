...
class ClientApiConnectionPacketStreamListener:
    ...
    async def __anext__(self) -> mesh_pb2.FromRadio:
        ...
        queue_task = asyncio.create_task(
            self._queue.get(), name="packet-stream-listener-queue-get"
        )
        closed_task = asyncio.create_task(
            self._closed.wait(), name="packet-stream-listener-closed"
        )
        try:
            done, pending = await asyncio.wait(
                [queue_task, closed_task], return_when=asyncio.FIRST_COMPLETED
            )
