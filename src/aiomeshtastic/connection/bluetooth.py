...
class BluetoothConnection(ClientApiConnection):
    ...
    async def _connect(self) -> None:
        ...
        self._ble_meshtastic_service = self._bleak_client.services[
        ...
        self._ble_log = self._ble_meshtastic_service.get_characteristic(
            BluetoothConnection.BTM_CHARACTERISTIC_LOG_UUID
        )
    ...
    async def _handle_notify_wait(  # noqa: PLR0913
        ...
        wait_force_read = asyncio.create_task(
            force_read_event.wait(), name="wait_force_read"
        )
        ...
        if wait_force_read in done:
            self._logger.debug(
                "Force read event received. Continuing loop for active read."
            )
    ...
    async def _packet_stream(self) -> AsyncGenerator[mesh_pb2.FromRadio, Any]:  # noqa: PLR0915
        ...
        try:
            ...
            async def start_notify() -> None:
                await asyncio.wait_for(
                    self._bleak_client.start_notify(
                        self._ble_from_num, notification_handler
                    ),
                    timeout=30,
                )
            ...
            async def stop_notify() -> None:
                await asyncio.wait_for(
                    self._bleak_client.stop_notify(self._ble_from_num), timeout=30
                )
            ...
            while True:
                ...
                if packet == b"":
                    ...
                    (
                        continue_active_read,
                        notify_timeout_count,
                    ) = await self._handle_notify_wait(
                        packet_num_queue,
                        force_read_event,
                ...
                try:
                    ...
                    self._logger.debug(
                        "Parsed packet: %s", self._protobuf_log(from_radio)
                    )
                    yield from_radio
                except message.DecodeError:
                    self._logger.warning(
                        "Error while parsing FromRadio bytes %s", packet, exc_info=True
                    )
    ...
    async def _send_packet(self, data: bytes) -> bool:
        ...
        try:
            ...
            self._logger.warning(
                "Could not parse ToRadio packet in _send_packet to check for want_config_id."
            )
